...
* Utilisation d'un **interceptor http** pour gérer les entête de requête / codes erreurs / retry...    
* Depuis Angular 13, vérifier que le répertoire **.angular/cache** est bien ajouté au fichier *.gitignore* ````/.angular/cache````       
* Sous Angular 14, utiliser les composants en mode standalone permet de s'affranchir des fichiers module     
...
* Utiliser au maximum les pipe ````async```` pour gérer automatiquement la souscription/désabonnement des observables  
* Observable ````pipe(take(1))```` ou conversion en promise lorsqu'un observable n'est pas nécessaire
* Sé désabonner systématiquement de chaque souscription manuelle à un observable
